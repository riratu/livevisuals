function setup() {
    createCanvas(windowWidth, windowHeight)
    w = windowWidth
    h = windowHeight
    rot = 0
    tStrH = 400
    strH = 200
}
function draw() {
    //Fill Background
    fill(0, 0, 0, 10)
    rect(0, 0, w, h)
    beat = millis() % 3000 / 1500 - 1
    invBeat = beat * -1
    //Rotate the Screen
    translate(windowWidth/2, windowHeight/2)
    rotate(rot)
    rot += 0.007
    if (frameCount % 100 == 0) tStrH = random(h)
    dh = tStrH -strH
    strH+= dh * 0.05
    stroke((beat + 1) * 255,(invBeat + 1) * 255, millis() % 10000 / 10000)
    for (i=0; i < 10; i++) {
        //line(random(30) + beat * h/2, strH,random(30) +  beat * w/2, strH * -1)
        circle(random(30) + beat * w/2, random(strH) -strH/2, random(4))
        lineX = random(30) + beat * w/2
        lineY =random(strH) -strH/2
        line(lineX + random(30), lineY + random(30), lineX + random(frameCount * 0.5 % 50 - 50), lineY + random(frameCount * 0.1 % 50 - 25))
    }
}